...
#ifndef PROJECTA_EDLIB_GWFA_HPP
#define PROJECTA_EDLIB_GWFA_HPP

// Struct to hold inputs for edlib_gwfa
struct projectA_edlib_gwfa_parameters_t {
    void* km;
    gwf_graph_t* graph;
    int32_t ql;
    const char *q;
...
struct projectA_edlib_gwfa_path_t {
    gwf_path_t path;
    int32_t score;
};

// Struct to hold the inputs and outputs for edlib_gwfa
struct projectA_edlib_gwfa_io_t {
    vector<vector<projectA_edlib_gwfa_parameters_t>> parameters;
    vector<vector<projectA_edlib_gwfa_path_t>> paths;
    int32_t size;
};
...
// POST:    return
//      return:         Void pointer that holds the populated structs needed for alignment by edlib_gwfa including reserved space for the results.
void* projectA_edlib_gwfa_init(vector<projectA_alignment_t*>& alignments, int32_t numThreads);


// PRE:     ptr
//      ptr:            Void pointer that holds the populated structs needed for alignment by edlib_gwfa including reserved space for the results.
// POST:    return
//      return:         Void pointer that holds the results of the edlib_gwfa alignment as well as the edlib_gwfa structs needed for alignment.
void* projectA_edlib_gwfa_calculate_batch(void* ptr, int32_t thread_index);


// PRE:     ptr, alignments
//      ptr:            Void pointer that holds the results of the edlib_gwfa alignment as well as the edlib_gwfa structs needed for alignment.
...
void projectA_edlib_gwfa_destroy(projectA_algorithm_t* gssw);
